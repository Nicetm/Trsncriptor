...
# Inicializar Pyannote Pipeline una sola vez
...
def transcribe_files(files, file_format):
    ...
    # Inicializar Pyannote y Audio
    pipeline = Pipeline.from_pretrained("pyannote/speaker-diarization")
    pipeline.segmentation.window = 2.0  # Ajustar la ventana a 2 segundos
    torch_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    pipeline.to(torch_device)  # Mover pipeline a GPU si está disponible
    audio_processor = Audio()  # Procesador de audio para obtener waveform y sample_rate
    ...
    for idx, file in enumerate(files):
        ...
        try:
            ...
            # Transcribir segmentos y combinar
            full_transcription = ""
            ...
            for i, segment in enumerate(segment_paths):
                ...
                transcription_with_timestamps.extend(transcription_segments)
                time.sleep(3)

            # Procesar con Pyannote (Diarización de hablantes)
            result_data[idx][1] = "Identificando hablantes..."
            file_registry[file_name]["status"] = "Identificando hablantes..."
            yield format_markdown_table(result_data), ""
            save_file_registry()

            waveform, sample_rate = audio_processor(file_path)  # Lee el archivo completo
            waveform = waveform.to(torch_device)  # Mueve waveform a GPU

            diarization = pipeline({"waveform": waveform, "sample_rate": sample_rate})

            # Asignar hablantes a segmentos de Whisper
            speaker_map = {}  # Mapa para estabilizar hablantes (ejemplo: SPEAKER_00 -> Hombre, SPEAKER_01 -> Mujer)
            next_speaker_id = 0

            for turn, _, speaker in diarization.itertracks(yield_label=True):
                # Verificar si el hablante ya está mapeado
                if speaker not in speaker_map:
                    speaker_map[speaker] = f"SPEAKER_{next_speaker_id}"
                    next_speaker_id += 1

                # Asignar etiquetas estabilizadas
                stable_speaker = speaker_map[speaker]

                # Asignar segmentos a este hablante
                for seg in transcription_with_timestamps:
                    if seg["start"] < turn.end and seg["end"] > turn.start:
                        seg["speaker"] = stable_speaker

            # Detectar segmentos no asignados
            for seg in transcription_with_timestamps:
                if "speaker" not in seg:
                    seg["speaker"] = "Narrador"

            # Generar transcripción con hablantes estabilizados
            full_transcription = "\n\n".join(
                [
